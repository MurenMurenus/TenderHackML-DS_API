...
async def get_curve(my_id):
    ...
    top_reg = (await metrics.get_top_region("1970-01-01", "2100-01-01", my_id)).head(1)['delivery_region']
    my_region = top_reg[0]
    my_lots = contracts_full_data[contracts_full_data['id'] == my_id]
    my_lots = my_lots.groupby('lot_name')['price_y'].sum().reset_index()
    my_lot_name = (my_lots[['lot_name', 'price_y']].sort_values(by='price_y', ascending=False)).head(1)['lot_name'][0]

    # boolean indexing to extract rows that match your criteria
    mask = (contracts_full_data['id'] == my_id) & \
    ...
    print(selected_rows_id)
    # assuming selected_rows is your DataFrame containing the selected rows
    sorted_rows_id = selected_rows_id.sort_values(by='contract_conclusion_date')
    ...
    sorted_rows_id = sorted_rows_id[['price_y', 'contract_conclusion_date']]
    print(sorted_rows_id.head())
    out = [[], []]
    out[0] = sorted_rows_id.to_json()
    print(out[0])
    ...
    mean_prices = mean_prices.to_frame()
    out[1] = mean_prices.to_json()
    print(out[1])
